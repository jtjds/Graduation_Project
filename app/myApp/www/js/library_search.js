...
angular.module('starter.search', [])
    .controller('library_searchCtrl', ['$scope', '$ionicPopup', '$timeout', '$stateParams', '$http', '$ionicScrollDelegate', '$location', '$window', '$ionicHistory', function ($scope, $ionicPopup, $timeout, $stateParams, $http, $ionicScrollDelegate, $location, $window, $ionicHistory) {
        ...
        $scope.searchMatch = function(){
            //console.log(JSON.stringify($scope.bookData.authorOrbookName));
            var objList1 = document.getElementById('ion-list-one');
            var objList2 = document.getElementById('ion-list-two');
            var search_none = document.getElementById('search_none');
            objList1.style.display = 'none';
            ...
            $http({
                ...
                for(var i=0; i<data.length;i++){
                    var str = data[i].bookName || data[i].author ;
                    var matchData = [];
                    search_none.style.display = 'none';
                    if(str.match($scope.bookData.authorOrbookName)){
                        matchData.push(data[i]); 
                        objList2.style.display = 'block';  
                        $scope.data = matchData;       
